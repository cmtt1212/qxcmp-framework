...
public class AdminStatisticPageController extends QXCMPController {
    ...
    public ModelAndView statisticPagesPage() {
        List<AccessHistoryPageResult> results = accessHistoryService.findByDateCreatedAfter(DateTime.now().minusDays(1).toDate(), new PageRequest(0, 20)).getContent().stream().sorted(Comparator.comparing(AccessHistoryPageResult::getNbr).reversed()).collect(Collectors.toList());
